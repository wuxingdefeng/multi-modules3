    ...
    function RestJs(serverURI) {
       ...
       function getSignature(accessKey, secretKey){
        ...
        function auth(digest,accessKey,secretKey,successCallback,failCallback){
	      	...
	      	var urlStr =  protocolStr + "//"+ self.restServer + "/rtc/auth/valid?callback=test&access_key=" + encodeURI(accessKey) + "&digest=" + digest;
			$.ajax({
			       type: "get",
			       url: urlStr,
			       dataType: "jsonp",
        ...
        function create(accessToken, topic, maxVideo,maxAudio,hostPassword,roomMode,successCallback,failCallback){
               var protocolStr = document.location.protocol;
        	       var urlStr =  protocolStr + "//"+ self.restServer + "/rtc/room/create?callback=test&owner_id=111111&access_tocken=" + accessToken + "&room_mode=" + roomMode+ "&topic=" + topic+ "&max_video=" + maxVideo+ "&max_audio=" + maxAudio+ "&host_password=" + hostPassword;
        	       $.ajax({
			       type: "get",
			       url: urlStr,
			       dataType: "jsonp",
